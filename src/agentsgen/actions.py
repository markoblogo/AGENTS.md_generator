...
def _status_for_file(
    ...
    if generated_sibling:
        findings.append(
            f"Generated sibling exists for {path.name}: {_generated_sibling_path(path, generated_suffix).name}"
        )

    return (
        StatusFileReport(True, markers_found, marker_sections, generated_sibling),
        findings,
        errors,
    )
